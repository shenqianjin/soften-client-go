...
func newConsumer(rtArgs *internal.RootArgs, cmdArgs *ConsumeArgs) *consumeService {
	...
	if len(c.concurrencyLimits) > 0 {
		for index, con := range c.concurrencyLimits {
			if con > 0 {
				c.concurrencyLimiters[fmt.Sprintf("Group-%d", index+1)] = limiter.NewCountLimiter(int(con)) // rate.New(int(li), time.Second)
			} else {
	...
	if len(c.quotaLimits) > 0 {
		for index, quota := range c.quotaLimits {
			if quota > 0 {
				c.quotaLimiters[fmt.Sprintf("Group-%d", index+1)] = limiter.NewCountLimiter(int(quota)) // rate.New(int(li), time.Second)
			} else {
